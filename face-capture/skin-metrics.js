/*
 * On-device "skin age" panel — the 15 consumer skin metrics shown in the mock
 * (Skin Type, Spots, Wrinkles, Texture, Acne, Dark Circles, Redness, Oiliness,
 * Moisture, Pores, Eye bags, Radiance, Firmness, Droopy Upper / Lower Eyelid)
 * plus an estimated Skin Age.
 *
 * It is a single-still image-processing HEURISTIC, not a dermatological
 * instrument. Everything runs in the browser on downscaled crops of the face
 * mesh regions; nothing is uploaded and no identity information is derived.
 *
 * The pore / wrinkle / mark primitives are the same ones as
 * src/lib/skinAnalysis.ts (box blur, high-pass, oriented line filters,
 * difference-of-Gaussians, connected components). The consumer metrics are
 * blends of those region signals plus a little landmark geometry for the
 * eyelids. Thresholds are first-pass and want calibration against graded
 * reference photos.
 *
 * Exposed as window.SkinMetrics = { analyze, METRICS }.
 */
;(function () {
  'use strict'
  ...
  function analyze(landmarks, source, sourceW, sourceH) {
    ...
    if (window.SkinMetrics && window.SkinMetrics._debug) {
      report.debug = { tShine, cShine, oiliness, cheekOil, dryness, moisture, skinType }
    }
    return report
  }
  ...
  window.SkinMetrics = { analyze, analyzeMap, METRICS, FACE_OVAL, REGIONS, polyFromIndices }
})()
